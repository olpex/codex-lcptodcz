...
test("workload auto refresh can be turned off and remembered", async ({ page }) => {
  ...
  await page.goto("/workload");
  await expect(page.getByRole("cell", { name: "Войтехівська Галина Михайлівна", exact: true })).toBeVisible();

  const autoRefresh = page.getByLabel("Автооновлення");
  await expect(autoRefresh).toBeChecked();
  await autoRefresh.uncheck();
  await expect(autoRefresh).not.toBeChecked();
  await expect.poll(() => page.evaluate(() => localStorage.getItem("suptc:workload:auto-refresh"))).toBe("false");

  await page.reload();
  await expect(page.getByRole("cell", { name: "Войтехівська Галина Михайлівна", exact: true })).toBeVisible();
  await expect(page.getByLabel("Автооновлення")).not.toBeChecked();

  await page.getByTestId("workload-refresh").click();
  await expect.poll(() => workloadRequests).toBeGreaterThanOrEqual(3);
});
